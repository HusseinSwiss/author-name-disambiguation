 ...
 *
 *  $Id: Factory.java 17866 2014-04-18 09:16:19Z markagreenwood $
 */
...
public abstract class Factory {
	...
	public static Resource createResource(String resourceClassName)
		...
		if (resData == null) {
			Set<DirectoryInfo> plugins = Gate
					.getDirectoryInfo(resourceClassName);

			StringBuilder msg = new StringBuilder();
			msg.append("Couldn't get resource data for ")
					.append(resourceClassName).append(".\n\n");
	...
	/**
	 * Create an instance of a resource, and return it. Callers of this method
	 * are responsible for querying the resource's parameter lists, putting
	 * together a set that is complete apart from runtime parameters, and
	 * passing a feature map containing these parameter settings.
	 * 
	 * @param resourceClassName
	 *            the name of the class implementing the resource.
	 * @param parameterValues
	 *            the feature map containing intialisation time parameterValues
	 *            for the resource.
	...
	public static Resource createResource(String resourceClassName,
			...
			throws ResourceInstantiationException {
		return createResource(resourceClassName, parameterValues, features,
				null);
	}
	 ...
	 *            for the resource.
	 * @param features
	 *            the features for the new resource or null to not assign any
	 *            (new) features.
	 * @param resourceName
	 *            the name to be given to the resource or null to assign a
	 *            default name.
	 * @return an instantiated resource.
	 */
	public static Resource createResource(String resourceClassName,
		...
		// if the object is an LR and it should come from a DS then create that
		// way
		DataStore dataStore;
		if (LanguageResource.class.isAssignableFrom(resClass)
			...
			if (dataStore instanceof SerialDataStore) {
				...
				if (!Serializable.class.isAssignableFrom(resClass))
					throw new ResourceInstantiationException(
							"Resource cannot be (de-)serialized: "
									+ resClass.getName());
			}

			// get the datastore instance id and retrieve the resource
			Object instanceId = parameterValues
					.get(DataStore.LR_ID_FEATURE_NAME);
			if (instanceId == null)
			...
			try {
				...
				throw new ResourceInstantiationException("Bad read from DB: "
						+ pe);
			} catch (SecurityException se) {
				throw new ResourceInstantiationException(
						"Insufficient permissions: " + se);
			}
		...
		if (LanguageResource.class.isAssignableFrom(resClass)) {
			...
			try {
				...
				parameters.putAll(resData.getParameterList()
						.getRuntimeDefaults());
		...
		// set the name
		// if we have an explicitly provided name, use that, otherwise generate
		// a
		...
		if (resourceName != null && resourceName.trim().length() > 0) {
			...
			try {
				...
				if (res instanceof SimpleDocument) {
					...
					java.lang.reflect.Method m = resClass
							.getMethod("getRdfXmlURL");
				...
				if (sourceUrl != null) {
					...
					if (resourceName == null || resourceName.length() == 0
						...
						// there is a significant path value -> get the last
						// element
						int lastSlash = resourceName.lastIndexOf('/');
						if (lastSlash >= 0) {
							String subStr = resourceName
		...
		if (listeners != null && !listeners.isEmpty()) {
			try {
				...
				if (DEBUG)
					Out.prln("Failed to remove the listeners for "
							+ res.toString());
				throw new ResourceInstantiationException(
						"Parameterisation failure" + e);
			}
	...
	/**
	 * Delete an instance of a resource. This involves removing it from the
	 * stack of instantiations maintained by this resource type's resource data.
	 * Deletion does not guarantee that the resource will become a candidate for
	 * garbage collection, just that the GATE framework is no longer holding
	...
	public static void deleteResource(Resource resource) {
		ResourceData rd = Gate.getCreoleRegister().get(
	...
	public static Corpus newCorpus(String name)
			throws ResourceInstantiationException {
		return (Corpus) createResource("gate.corpora.CorpusImpl",
				newFeatureMap(), newFeatureMap(), name);
	} // newCorpus
	...
	public static Document newDocument(URL sourceUrl, String encoding)
		...
		parameterValues.put(Document.DOCUMENT_URL_PARAMETER_NAME, sourceUrl);
		parameterValues
				.put(Document.DOCUMENT_ENCODING_PARAMETER_NAME, encoding);
		return (Document) createResource("gate.corpora.DocumentImpl",
				parameterValues);
	} // newDocument(URL)
	...
	public static Document newDocument(String content)
		...
		Document doc = (Document) createResource("gate.corpora.DocumentImpl",
				params);
		/*
		 * // laziness: should fit this into createResource by adding a new //
		 * document parameter, but haven't time right now... doc.setContent(new
		 * DocumentContentImpl(content));
		 */
	 ...
	 * @param annotations
	 *            the set of annotations that should be contained in the
	...
	/**
	 * <p>
	 * Create a <i>duplicate</i> of the given resource. A duplicate is a an
	 * independent copy of the resource that has the same name and the same
	 * behaviour. It does <i>not necessarily</i> have the same concrete class as
	 * the original, but if the original resource implements any of the
	 * following interfaces then the duplicate can be assumed to implement the
	 * same ones:
	 * </p>
	 * <ul>
	 ...
	 * createResource} with the type and name of the original resource, and with
	 * parameters and features which are copies of those from the original
	 * resource, but any Resource values in the maps will themselves be
	 * duplicated. A context is passed around all the duplicate calls that stem
	 * from the same call to this method so that if the same resource is
	 * referred to in different places, the same duplicate can be used in the
	 * corresponding places in the duplicated object graph.
	 * </p>
	 * <p>
	 * This default behaviour is sufficient for most resource types (and is
	 * roughly the equivalent of saving the resource's state using the
	 ...
	 * interface. This may be necessary for semantic reasons (e.g. controllers
	 * need to recursively duplicate the PRs they contain), or desirable for
	 * performance or memory consumption reasons (e.g. the behaviour of a
	 * DefaultGazetteer can be duplicated by a SharedDefaultGazetteer that
	 * shares the internal data structures).
	 * </p>
	 * 
	 * @param res
	 *            the resource to duplicate
	 * @return an independent duplicate copy of the resource
	 * @throws ResourceInstantiationException
	 *             if an exception occurs while constructing the duplicate.
	 */
	 ...
	 * {@link CustomDuplication} interface when they need to duplicate their
	 * child resources. Calls made to this method outside the scope of such a
	 * {@link CustomDuplication#duplicate CustomDuplication.duplicate} call will
	 ...
	 * @throws ResourceInstantiationException
	 *             if an error occurs while constructing the duplicate.
	 */
	public static Resource duplicate(Resource res, DuplicationContext ctx)
		...
		try {
			...
			// check if we've seen this resource before
				...
				if (DEBUG_DUPLICATION) {
					log.debug(myDupIndex
							+ ": Resource already duplicated in context");
				}
	...
	/**
	 * Implementation of the default duplication algorithm described in the
	 * comment for {@link #duplicate(Resource)}. This method is public for the
	 * benefit of resources that implement {@link CustomDuplication} but only
	 * need to do some post-processing after the default duplication algorithm;
	 * they can call this method to obtain an initial duplicate and then
	 * post-process it before returning. If they need to duplicate child
	 * resources they should call
	 * {@link #duplicate(Resource, DuplicationContext)} in the normal way. Calls
	 * to this method made outside the context of a
	 * {@link CustomDuplication#duplicate CustomDuplication.duplicate} call will
	 * fail with a runtime exception.
	 * 
	 * @param res
	 ...
	 * @param ctx
	 *            the current context
	 * @return a duplicate of the given resource, constructed using the default
	 *         algorithm. In particular, if <code>res</code> implements
	 *         {@link CustomDuplication} its own duplicate method will
	 *         <i>not</i> be called.
	 * @throws ResourceInstantiationException
	 *             if an error occurs while duplicating the given resource.
	 */
	public static Resource defaultDuplicate(Resource res, DuplicationContext ctx)
		...
		Resource newResource = createResource(className, initParams,
				newResFeatures, resName);
		if (newResource instanceof ProcessingResource) {
			// runtime params
			FeatureMap runtimeParams = AbstractProcessingResource
					.getRuntimeParameterValues(res);
			// remove parameters that are also sharable properties
			for (String propName : resData.getSharableProperties()) {
	...
	/**
	 * Construct a feature map that is a copy of the one provided except that
	 * any {@link Resource} values in the map are replaced by their duplicates.
	 * This method is public for the benefit of resources that implement
	 * {@link CustomDuplication} and will fail if called outside of a
	 * {@link CustomDuplication#duplicate CustomDuplication.duplicate}
	 * implementation.
	 * 
	 * @param fm
	 *            the feature map to duplicate
	 * @param ctx
	 *            the current context
	 * @return a duplicate feature map
	 * @throws ResourceInstantiationException
	 *             if an error occurs while duplicating any Resource in the
	 *             feature map.
	 */
	 ...
	 * CustomDuplication.duplicate} methods to encapsulate the state of the
	 * current duplication run. If the duplicate method itself needs to
	 * duplicate any objects it should pass this context back to
	 * {@link #duplicate(Resource,DuplicationContext)}.
	 */
	public static class DuplicationContext {
		IdentityHashMap<Resource, Resource> knownResources = new IdentityHashMap<Resource, Resource>();
	...
	/**
	 * Throws an exception if the specified duplication context is null or not
	 * active. This is to ensure that the Factory helper methods that take a
	 * DuplicationContext parameter can only be called in the context of a
	 ...
	 *             if the provided context is not active.
	 */
	...
	public static ParseCpsl newJapeParser(java.io.Reader stream,
		...
		try {
			Constructor<? extends ParseCpsl> c = japeParserClass
					.getConstructor(new Class<?>[] { Reader.class, Map.class });
	...
	public static ParseCpsl newJapeParser(URL japeURL, String encoding)
			throws IOException {
		// the stripping stream is buffered, no need to buffer the URL stream.
		java.io.Reader stream = new BomStrippingInputStreamReader(
				japeURL.openStream(), encoding);
	 ...
	 * creation is an system administrator task; in such cases this method will
	...
	protected static DataStore instantiateDataStore(String dataStoreClassName,
		...
		try {
			godfreyTheDataStore = (DataStore) Gate.getClassLoader()
...
/**
 * Factory is basically a collection of static methods but events need to have
 * as source an object and not a class. The CreolProxy class addresses this
 * issue acting as source for all events fired by the Factory class.
 */
class CreoleProxy {
	...
	public synchronized void removeCreoleListener(CreoleListener l) {
		if (creoleListeners != null && creoleListeners.contains(l)) {
			...
			Vector<CreoleListener> v = (Vector<CreoleListener>) creoleListeners
					.clone();
			v.removeElement(l);
			creoleListeners = v;
		}// if
	...
	public synchronized void addCreoleListener(CreoleListener l) {
		@SuppressWarnings("unchecked")
		Vector<CreoleListener> v = creoleListeners == null ? new Vector<CreoleListener>(
				2) : (Vector<CreoleListener>) creoleListeners.clone();
		if (!v.contains(l)) {
			v.addElement(l);
			creoleListeners = v;
		}// if
